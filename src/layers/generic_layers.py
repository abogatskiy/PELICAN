...
class InputEncoder(nn.Module):
    def __init__(self, out_dim, device=torch.device('cpu'), dtype=torch.float):
        ...
        self.to(device=device, dtype=dtype)
        self.alphas = nn.Parameter(1.05 * torch.linspace(0, 1.05, out_dim, device=device, dtype=dtype).view(1, 1, 1, out_dim)) #torch.rand(1, 1, 1, out_dim, device=device, dtype=dtype))
        self.zero = torch.tensor(0, device=device, dtype=dtype)
